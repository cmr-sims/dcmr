...
def main(data_file, patterns_file, param1, sweep1, param2, sweep2, res_dir, n_rep=1):
    ...
    param_def.set_free(
        {param1: (sweep1[0], sweep1[-1]), param2: (sweep2[0], sweep2[-1])}
    )
    ...
    results = model.parameter_sweep(
        study_data, param_def, param_names, param_sweeps, patterns=patterns, n_rep=n_rep
    )
    ...
    g = fr.plot_spc(p.reset_index(), row=param_names[0], col=param_names[1], **kws)
    ...
    g = fr.plot_lag_crp(p.reset_index(), row=param_names[0], col=param_names[1], **kws)
    ...
    g = fr.plot_raster(
        fr.reset_list(sim1.reset_index()),
        row=param_names[0],
        col=param_names[1],
