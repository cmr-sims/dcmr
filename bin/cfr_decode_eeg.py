...
def decode_subject(patterns_dir, subject):
    ...
    evidence = decode.classify_patterns(events, pattern)
...
def main(patterns_dir, n_jobs=1, subjects=None):
    if subjects is None:
        subjects, _ = task.get_subjects()

    Parallel(n_jobs=n_jobs)(
        delayed(decode_subject)(patterns_dir, subject) for subject in subjects
    )
...
if __name__ == '__main__':
    ...
    if args.subjects is not None:
        ...
        inc_subjects = None
    main(args.patterns_dir, args.n_jobs, inc_subjects)
