function data = run_res_indiv_best_params_cfrl(experiment, fit, proj_dir, ...
                                               res_dir, res_file, varargin)
%RUN_RES_INDIV_BEST_PARAMS_CFR   Run simulations with best-fitting parameters.
%
%  Load the best-fitting parameters from a search and run the
%  generative version of the model to create synthetic data. This
%  is a modified version of run_indiv_best_params_cfrl where you
%  tell it where the results structure is instead of the function
%  figuring out where it is and loading it from disk.
%
%  data = run_res_indiv_best_params_cfrl(experiment, fit, ...)
%
%  OPTIONS:
%  n_rep - int - 100
...
stats = getfield(load(fullfile(res_dir,res_file), 'stats'), 'stats');
...
[~,name,extension] = fileparts(res_file);
stat_filename = [name '_stats.mat'];
stat_path = fullfile(res_dir, stat_filename);
fprintf('Saving statistics in: %s\n', stat_path);
save(stat_path, 'data', 'custom', 'simdef');
