...
def plot_fit(data, group_var, stat_name, f_stat, stat_kws, var_name,
             f_plot, plot_kws, out_dir):
    ...
    g = sns.relplot(kind='scatter', x=groups[0], y=groups[1], hue=cond_name,
...
def plot_fit_scatter(data, group_var, stat_name, f_stat, stat_kws, var_name,
                     out_dir):
    ...
    stat = stat[[var_name]]
    comp = stat.unstack(level=0).droplevel(axis=1, level=0)
    g = sns.relplot(kind='scatter', x=groups[0], y=groups[1],
                    data=comp.reset_index(), height=4)
    g.axes[0, 0].plot([0, 1], [0, 1], '-k')
    g.savefig(os.path.join(out_dir, f'{stat_name}_comp_subject.pdf'))
    plt.close(g.fig)
