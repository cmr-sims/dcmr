...
def plot_support(net, item_cues=None, sublayers=None, **kwargs):
    """Plot support for items based on item cues."""
    support = framework.model_cue_support(net, item_cues=item_cues, sublayers=sublayers)
...
def plot_fit(
    ...
    for var in check_var:
        if isinstance(comp[var].dtype, pd.ArrowDtype):
            if pd.api.types.is_integer_dtype(comp[var]):
